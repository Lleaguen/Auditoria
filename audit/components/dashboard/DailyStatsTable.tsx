...
export default function DailyStatsTable({ data }: DailyStatsTableProps) {
  ...
  const totals = data.reduce(
    (acc, d) => ({
      ...
      totalSurplus: acc.totalSurplus + d.totalSurplus,
      ...
      <table className="w-full text-xs">
        <thead className="bg-zinc-800 text-zinc-200 sticky top-0">
          <tr>
            ...
            <th className="px-3 py-2.5 text-right font-semibold">Q Envíos Tot. Auditados</th>
            <th className="px-3 py-2.5 text-right font-semibold">Q Envíos Faltantes</th>
            <th className="px-3 py-2.5 text-right font-semibold">Q Envíos Sobrantes</th>
            <th className="px-3 py-2.5 text-right font-semibold">Q Envíos Dañados</th>
            ...
            <tr key={d.date} className="hover:bg-zinc-50">
              ...
              <td
                className={`px-3 py-2 text-right font-semibold ${
                  d.percentHusWithDeviation > 50 ? 'text-red-600' : 'text-zinc-700'
                }`}
              >
              ...
              <td className="px-3 py-2 text-right text-orange-600">{d.totalSurplus}</td>
              <td className="px-3 py-2 text-right text-zinc-500">{d.totalDamaged}</td>
              <td
                className={`px-3 py-2 text-right font-semibold ${
                  d.percentWithErrors > 5 ? 'text-red-600' : 'text-zinc-700'
                }`}
              >
        ...
        <tfoot className="bg-zinc-100 border-t-2 border-zinc-300 font-semibold">
          <tr>
            ...
            <td className="px-3 py-2 text-right text-orange-600">{totals.totalSurplus}</td>
            <td className="px-3 py-2 text-right text-zinc-800">{totals.totalDamaged}</td>
            <td className="px-3 py-2 text-right text-zinc-800">
              {totals.totalShipmentsAudited > 0
                ? (
                    ((totals.totalMissing + totals.totalSurplus) /
                      totals.totalShipmentsAudited) *
                    100
                  ).toFixed(2)
                : '0.00'}%
            </td>
